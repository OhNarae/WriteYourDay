...
$(document).ready(function() {
	...
	$.ajax({
		...
		success: function(out){
            ...
            $('#pop-friends').append('<option value="'+item.seq+'">'+item.id+'</option>');})
		}
	})	
})

function layer_popup(seq) { //#layer
	$('.dim-layer').fadeIn();
	...
	$('#layer').find('a.btn-layer').attr('href','javascript:isShare(' + seq + ')');
}

function isShare(seq) {
	if (confirm("정말 이벤트를 공유하시겠습니까?")) {
		$.ajax({
			type : 'Post',
			url : '/WriteYourDay/event/share/insert.do',
			data : {
				event_seq: seq,
				you_member_seq: $('#pop-friends option:selected').val()				
			},
			success : alert('공유를 요청 하였습니다.')
		})
		$('.dim-layer').fadeOut(); // 닫기 버튼을 클릭하면 레이어가 닫힌다.
	} else {
		$('.dim-layer').fadeOut(); // 닫기 버튼을 클릭하면 레이어가 닫힌다.
...
function eventlist() {
	$.ajax({
		...
		success : function(out) {
			$('#eventT-body').html('');
			$.each(out.result, function(i, item) {
				var tr = '<tr style="background-color:' + item.color + '">';
				tr += '<td>' + item.title + '</td>'
	...
	$.ajax({
		...
		success : function(out) {
			...
			$.each(out.result, function(i, item) {
				...
				tr += '<td>' + item.price + '<a class="fright" href="javascript:deleteCashbook(' + item.seq + ')">삭제</a></td></tr>';
